...
import { ISpoonRecipe, SpoonEquipment } from 'src/spoon/models/spoon-random-recipe.dto';
...
import { PaginatedDto, PageMetaDto } from 'src/base/paginated.dto';
import { IRecipe, IRecipeShort, THealthBooleanLabels } from './recipe.dto';
...
import { IEquipmentSteppedInstruction } from './recipe-stepped-instructions/equipment-stepped-instruction.dto';
...
export class RecipeService {
  ...
  async getRecipeById(id: number): Promise<IRecipe | IMessage> {
    ...
    if (!getRecipe || getRecipe === null) {
      ...
      return new IMessage('Recipe by id not found, ' + queryTime + 'ms', HttpStatus.NOT_FOUND);
    }
  ...
  private mapRecipeSteppedInstructionToISteppedInstruction(si: RecipeSteppedInstruction, recipeId: number): IRecipeSteppedInstruction {
    return {
      ...
      ingredients: si.ingredients.map((i: Ingredient) => new IIngredientShort(i)),
