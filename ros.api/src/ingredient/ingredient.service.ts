...
import { EOrder, PageMetaDto, PaginatedDto } from 'src/base/paginated.dto';
...
import { ISpoonIngredient } from 'src/spoon/models/spoon-ingredient.dto';
...
import { EPurchasedBy, ICaloricBreakdown, IIngredient, INutrients, INutritionProperties, IReferenceShort } from './ingredient.dto';
import { Ingredient } from './ingredient.entity';
import { RecipeIngredient } from 'src/recipe/recipe-ingredient/recipe-ingredient.entity';
import { IRecipeIngredient } from 'src/recipe/recipe-ingredient/recipe-ingredient.dto';
import { IIngredientShort } from './ingredient-short.dto';
...
export class IngredientService {
  ...
  async createIngredientFromDto(ingredient: IIngredient): Promise<Ingredient | IMessage> {
    const nameAvailable = await this.ingredientNameAvailable(ingredient.name);

    if (!nameAvailable) {
      return new IMessage('Ingredient name already exists', HttpStatus.CONFLICT);
    }

    if (ingredient.externalId) {
      const spoonCheck = await this.repository.findOne({ where: { externalId: ingredient.externalId } });
      if (!!spoonCheck) {
        return new IMessage('Ingredient with same spoon identity already exists', HttpStatus.CONFLICT);
      }
    }
  ...
  async spoonIngredientIdExists(externalId: number): Promise<Ingredient | IMessage> {
    const spoonCheck = await this.repository.findOne({ where: { externalId } });

    return !!spoonCheck ? spoonCheck : new IMessage('Go ahead and create it', HttpStatus.OK);
  }

  /** The name must also be unique. */
  async spoonIngredientNameExists(spoon: ISpoonIngredient): Promise<Ingredient | IMessage> {
  ...
  async createIngredientFromSpoon(spoon: Ingredient): Promise<Ingredient | IMessage> {
    const result = await this.repository.save(spoon);

    return result !== null ? result : new IMessage('something went pear shaped', HttpStatus.INTERNAL_SERVER_ERROR);
  }
  ...
  mapRecipeIngredientToIRecipeIngredient(
    ...
    return {
      ...
      ingredient: new IIngredientShort(this.findIngredient(iL.ingredientId, ingredients))
    };
  }
